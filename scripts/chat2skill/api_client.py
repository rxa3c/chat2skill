...
Conversations and the user's LLM credential are used in memory for the one
...
def _post_json(url: str, payload: dict, timeout: int) -> dict:
    ...
    try:
        ...
        detail = _redact(exc.read().decode("utf-8", errors="replace")[:500], credentials)
        raise ApiError(f"HTTP {exc.code} from {url}: {detail}") from None
    except urllib.error.URLError as exc:
        raise ApiError(f"cannot reach {url}: {_redact(str(exc.reason), credentials)}") from None
    except json.JSONDecodeError:
        raise ApiError(f"invalid JSON response from {url}") from None


def _payload_credentials(payload: dict) -> tuple[str, ...]:
    llm = payload.get("llm")
    if isinstance(llm, dict):
        return tuple(
            value
            for value in (llm.get("api_key"), llm.get("access_token"))
            if isinstance(value, str) and value
        )
    return ()


def _redact(text: str, credentials: Iterable[str]) -> str:
